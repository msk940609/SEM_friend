...
transparent=function(size=0){
  ...
  temp=theme(rect= element_rect(fill = 'transparent',size=size),
             panel.background=element_rect(fill = 'transparent', color = NA, size = 0),
...
#'@param fill option for pie color
#'@param fillcol pie color list
#'@param subfill option for donut color
#'@param subfillcol donut color list
#'@param subfillcol donut color list
#'@param pielwd pie line width
#'@param donutlwd donut line width
#'@param color color
...
#'}
PieDonut_ms=function(data,mapping,
  ...
  #data=fm_wsoc_b
  #mapping=aes(pies=type,donuts=NOS2)
  ...
  if(showRatioPie) {
    ...
    df$label=ifelse(df$ratio>=showRatioThreshold,
                    paste0(df$label,"\n(",scales::percent(df$ratio,accuracy = 0.1),")"),
  ...
  #  subColor=makeSubColor(mainCol,no=length(unique(data[[donuts]])))
   # subColor
    ...
    df3
    if(ratioByGroup) {
      df3$ratio=scales::percent(df3$Freq/df3$group,accuracy = 0.1)
    } else {
      df3$ratio<-scales::percent(df3$ratio1,accuracy = 0.1)
    }
  ...
  p <- ggplot() + theme_no_axes() + coord_fixed()+theme(rect= element_rect(fill = 'transparent',size=0,color = NA),
                                                        panel.background=element_rect(fill = 'transparent', color = NA, size = 0),
                                                        plot.background = element_rect(fill = "transparent", color = NA, size=0), # bg of the plot
                                                        panel.border=element_blank(),
                                                        panel.grid.major=element_blank(),
  ...
  p1<-p + geom_arc_bar(aes_string(x0 = "x", y0 = "y",
                                  ...
                                  fill = pies),alpha=pieAlpha,color=piecolor,size=pielwd,
                       data = df)+transparent()+
  ...
  if((labelposition==1)&(is.null(donuts))){
    ...
    p1 <-p1+geom_text(aes_string(x="labelx",y="labely",label="label"),size=pieLabelSize,data=df,family=family,color=color)
  }
  
  if(showPieName) p1<-p1+annotate("text",x=0,y=0,label=pietitle,size=titlesize,family=family)
  p1<-p1+theme(text=element_text(family=family, face = "bold"))
  ...
  if(!is.null(donuts)){
    ...
    if(explodeDonut) {
      p3<-p+geom_arc_bar(aes_string(x0 = "x", y0 = "y", r0 = as.character(r1),
                                    ...
                                    fill = "no",explode="focus"),alpha=donutAlpha,color=donutcolor, size=donutlwd,
                         data = df3)
    } else{
      p3<-p+geom_arc_bar(aes_string(x0 = "x", y0 = "y", r0 = as.character(r1),
                                    r = as.character(r2), start="start1",end="end1",
                                    fill = "no"),alpha=donutAlpha,color=donutcolor, size=donutlwd,
                         data = df3)
    }
    ...
    if(labelposition==1){
      ...
      p3<-p3+geom_text(aes_string(x="labelx",y="labely",
                                  label="label"),size=donutLabelSize,data=df3,family=family, color=NA)
    } else{
      p3<-p3+ geom_segment(aes_string(x="segx",y="segy",
